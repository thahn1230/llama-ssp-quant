...
@torch.no_grad()
def quantize_weight_per_channel_absmax(w, n_bits=6):
...
@torch.no_grad()
def quantize_weight_per_tensor_absmax(w, n_bits=6):
    # w: (out_features, in_features)
    scales = w.abs().max()
    q_max = 2 ** (n_bits - 1) - 1
    scales.clamp_(min=1e-5).div_(q_max)
...
@torch.no_grad()
def quantize_activation_per_token_absmax(t, n_bits=6):
    t_shape = t.shape
    t.view(-1, t_shape[-1])
...
@torch.no_grad()
def quantize_activation_per_tensor_absmax(t, n_bits=6):
    t_shape = t.shape
    t.view(-1, t_shape[-1])
    scales = t.abs().max()
    q_max = 2 ** (n_bits - 1) - 1
