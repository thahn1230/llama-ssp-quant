...
@torch.no_grad()
def quantize_weight_per_channel_absmax(w, n_bits=7):
...
@torch.no_grad()
def quantize_weight_per_tensor_absmax(w, n_bits=7):
    # w: (out_features, in_features)
    scales = w.abs().max()
    q_max = 2 ** (n_bits - 1) - 1
    scales.clamp_(min=1e-5).div_(q_max)
...
@torch.no_grad()
def quantize_activation_per_token_absmax(t, n_bits=7):
    t_shape = t.shape
    t.view(-1, t_shape[-1])
...
@torch.no_grad()
def quantize_activation_per_tensor_absmax(t, n_bits=7):
    t_shape = t.shape
    t.view(-1, t_shape[-1])
    scales = t.abs().max()
    q_max = 2 ** (n_bits - 1) - 1
...
class W8A8Linear(nn.Module):
    def __init__(
        ...
        if act_quant == "per_token":
            ...
            self.act_quant = partial(quantize_activation_per_token_absmax, n_bits=7)
        elif act_quant == "per_tensor":
            self.act_quant_name = "per_tensor"
            self.act_quant = partial(quantize_activation_per_tensor_absmax, n_bits=7)
        else:
